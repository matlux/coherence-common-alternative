...
public class DefaultConfigurationContext implements ConfigurationContext, EnrichmentSupport
{
                                            ...
                                            QualifiedName... except) throws ConfigurationException
    {
            ...
            .hasNext();)
        {
            ...
            {
                String id = childElement.getAttributeMap().containsKey("id") ? childElement.getAttribute("id")
                    .getString() : new UUID().toString();
                if (id.trim().length() == 0)
                {
            ...
            .hasNext();)
        {
            ...
            else
            {
                throw new ConfigurationException(String.format(
                    "The child element %s of the %s element did not produce the expected type %s when processed.",
                    childElement, xmlElement, expectedType), String.format(
                    "Please consult the documentation regarding use of the '%s' namespace", new QualifiedName(
                        xmlElement).getPrefix()));
            }
        }
                ...
                if (method.getParameterTypes().length == 1)
                {
                    ...
                    if (isPropertyDefined(propertyName, qualifiedName, xmlElement))
                    {
                        ...
                        Class<?> propertyType = method.getAnnotation(Type.class) == null
                                ? method.getParameterTypes()[0] : method.getAnnotation(Type.class).value();
                        ...
                        catch (Exception exception)
                        {
                            ...
                            throw new ConfigurationException(
                                String.format(
                                    ...
                                    getNamespaceContentHandler(qualifiedName.getPrefix()).getClass().getName()),
                                exception);
                        }
                    }
                    ...
                    {
                        throw new ConfigurationException(String.format(
                            "The mandatory property '%s' is not defined in element '%s'.", propertyName, xmlElement),
                            String.format("Please consult the documentation for the use of the %s namespace",
    ...
    /**
     * <p>Determine whether a method is marked as mandatory via the {@link Mandatory}
     ...
     */
    protected boolean isMandatory(final Method m, final XmlElement element)
    {
                    ...
                    if (propertyValue != null)
                    {
                        ...
                        if (propertyType.isAssignableFrom(propertyValue.getClass()))
                        {
                            ...
                            if (propertySubType != null)
                            {
                                ...
                                if ((propertyValue instanceof ReflectiveBuilder<?> && !((ReflectiveBuilder<?>) propertyValue)
                                    .realizesClassOf(propertySubType, SystemPropertyParameterProvider.INSTANCE))
                                        || (!(propertyValue instanceof ReflectiveBuilder<?>) && !propertySubType
                                            .isAssignableFrom(propertyValue.getClass())))
                                {
                                    ...
                                    throw new ConfigurationException(
                                        ...
                                        String
                                            .format(
                                                "The namespace implementation '%s' will need to programmatically configure the said property.",
                                                getNamespaceContentHandler(qualifiedName.getPrefix()).getClass()
                                                    .getName()));
                                }
                            }
                        }
                        ...
                        //is the property value a class scheme that can be realized now to produce the expected type?
                                ...
                                && ((ParameterizedBuilder<?>) propertyValue).realizesClassOf(propertyType,
                                    SystemPropertyParameterProvider.INSTANCE))
                        {
                            propertyValue = (T) ((ParameterizedBuilder<?>) propertyValue)
