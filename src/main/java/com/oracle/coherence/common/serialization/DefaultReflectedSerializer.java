...
final class DefaultReflectedSerializer implements ReflectedSerializer
{
                            ...
                            int version)
    {
        ...
        Arrays.sort(alphabeticalFields, new Comparator<Field>()
        {
                               ...
                               Field field2)
            {
                ...
                String fieldName1 = pofField1 == null ? field1.getName() : pofField1.name().isEmpty() ? field1
                    .getName() : pofField1.name();

                PofField pofField2 = field2.getAnnotation(PofField.class);
                String fieldName2 = pofField2 == null ? field2.getName() : pofField2.name().isEmpty() ? field2
                    .getName() : pofField2.name();
        ...
        for (int iSince = 0; iSince <= version; iSince++)
        {
            for (Field field : alphabeticalFields)
            {
                ...
                else if (includeField(field, iSince))
                {
                    ...
                    FieldMetaInfo fieldMetaInfo = new FieldMetaInfo(field, nextPofIndex++, preferredName,
                        preferredType, iSince, serializer);
        ...
        NoSuchMethodException
    {
        ...
        if (type.getSuperclass().isAnnotationPresent(PofType.class))
        {
            DefaultReflectedSerializer pofSerializer = (DefaultReflectedSerializer) reflectedPofSerializer
        ...
        {
            fieldMetaInfo.getFieldSerializer().readField(object, fieldMetaInfo.getField(), reader,
                          ...
                          Object object) throws IOException
    {
        ...
        else
        {
            ...
            for (FieldMetaInfo fieldMetaInfo : fieldMetaInfoByVersion.get(serializingVersion))
            {
                try
                {
                    fieldMetaInfo.getFieldSerializer().writeField(object, fieldMetaInfo.getField(), writer,
                        fieldMetaInfo.getPofIndex());
                }
                catch (Exception e)
                {
